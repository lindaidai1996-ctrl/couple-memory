...
test('buildPublicMetadata builds home metadata from public space fields', () => {
  ...
  assert.equal(metadata.alternates?.canonical, '/story/sun-moon')
  assert.equal(metadata.openGraph?.url, '/story/sun-moon')
...
test('resolvePublicMetadata loads a slug and builds page metadata', async () => {
  ...
  assert.equal(metadata.alternates?.canonical, '/story/sun-moon/photos')
})
