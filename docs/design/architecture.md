...
>
> 相关文档：[总览](./README.md) · [数据库设计](./database.md) · [Agent 编排设计](./ai-agent.md) · [部署方案](./deployment.md)
...
┌────────┐ ┌─────────────────────────────────┐
│PostgreSQL│ │       外部 AI 服务                 │
│ Prisma  │ │  Qwen / OpenAI / Claude / DeepSeek│
└────────┘ │  高德地图 API (逆地理编码)         │
            └─────────────────────────────────┘
```
...
4. 图片处理主链调用 Agent 编排层，按固定 DAG 执行视觉分析、文案、排版和时间线建议
5. 运行结果写回 `Photo`、`PhotoAIVariant`、`PipelineRun`
6. 公开页面通过 SSR/SSG 渲染，图片从 CDN 分发
...
### 多供应商视觉模型 + DeepSeek — AI 模型

**为什么是多供应商路由**：当前代码优先使用 Qwen 或 OpenAI 视觉能力，在缺少对应密钥或请求失败时再回退到 Claude，文案节点最终还会回退到 DeepSeek 文本生成。这样做不是为了抽象而抽象，而是为了在不同部署环境下保持链路可用，并把成本较高的视觉调用限制在必要节点。

详见 [Agent 编排设计](./ai-agent.md)。
...
**理由**：MVP 阶段只有一个应用，Monorepo 增加不必要的构建复杂度。`src/lib/agents/engine/` 目录保持零外部依赖，如果未来需要独立发布为 npm 包，可以直接拆出。
...
**决策**：自建轻量级 DAG Pipeline Engine，并把运行结果落库为 `PipelineRun`

**理由**：现有框架（LangChain、LlamaIndex）偏重 LLM 自主编排，引入大量不需要的依赖。当前业务只需要固定 DAG、按层并发、失败降级和节点级可观测性；自建引擎更适合承载这些约束，也更容易和 `Photo`、`PhotoAIVariant`、`PipelineRun` 这套持久化结构对接。
